...
struct Pertag {
  ...
  int enablegaps[LENGTH(tags) + 1]; /* if gaps are enabled for tag */
};
...
#endif /* __OpenBSD__ */
	scan();
	restoreSession(); // not sure if this should be before or after runAutostart so if something goes wrong look here lol
	runAutostart();
	run();
