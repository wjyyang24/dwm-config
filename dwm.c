...
drawbar(Monitor *m)
{
	...
	if (m->num == 0) { /* status is only drawn on first monitor 
			      not sure how the monitor num is determined tbh */
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MapRequest:
			...
			break;
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / 240)) /* change limit to 240 fps */
				continue;
